...
class CyclexAuthController(http.Controller):
    ...
    def login(self, **kwargs):
        ...
        try:
            ...
            _logger.info(f"Login attempt for phone: {phone}")
            ...
            if not partner:
                _logger.warning(f"Login failed: partner not found for phone {phone}")
                return request.make_response(
                    json.dumps({
                        'success': False,
            ...
            if user:
                ...
                try:
                    _logger.info(f"Authenticating user {user.login} (partner {partner.id}) via session.authenticate")
                    credential = {
                        'type': 'password',
                        'login': user.login,
                        'password': password,
                    ...
                    uid = (auth_info or {}).get('uid') or request.session.uid
                    _logger.info(f"Authentication result for {user.login}: {auth_info}")
                    if not uid:
                    ...
                    # Ensure environment uses the authenticated user
                    request.env = request.env(user=uid)
                except AccessDenied:
                    _logger.warning(f"Authentication denied for {user.login}")
                    return request.make_response(
                        json.dumps({
                            'success': False,
                            'message': _('Invalid phone number or password'),
                    ...
                    _logger.error(f"Authentication exception for {user.login}: {e}")
                    return request.make_response(
                        json.dumps({
                            'success': False,
                            'message': _('Invalid phone number or password'),
                ...
                _logger.warning(f"Login failed: no res.users record for partner {partner.id}")
            ...
            _logger.info(f"Login successful for partner {partner.id}")
    ...
    @http.route('/api/cyclex/profile', type='http', auth='user', methods=['POST'], csrf=False)
